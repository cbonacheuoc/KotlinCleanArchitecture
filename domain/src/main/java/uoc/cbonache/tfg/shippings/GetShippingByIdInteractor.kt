 ...
 * @author cbonache
 */
class GetShippingByIdInteractor @Inject constructor(postExecutionThread: PostExecutionThread,
                                                   val repository: ShippingsRepository,
                                                   val getTokenInteractor: GetTokenInteractor)

    : Interactor<Shipping, GetShippingByIdInteractor.Parameters>(postExecutionThread) {

    override fun run(params: Parameters): Result<Shipping, *> {

        val result = this.getToken().map { value ->
            value.access_token }.flatMap { value ->
            repository.getShippingById(value, params.id) }
        return result
    }
